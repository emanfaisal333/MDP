...
class GridWorldMDP:
    def __init__(self, rows=4, cols=4, gamma=0.9):
        ...
        self.prob_success = 0.8  # 80% success rate
        self.prob_fail = 0.2     # 20% failure/slip rate
        
        # Terminal states (Goal and Trap)
    ...
    def reset_env(self):
        """Generates new obstacles and resets values. Called for a fresh start."""
        # Places obstacles in specific rows as requested: Last (3) and Third-Last (1)
        self.obstacles = self._generate_row_specific_obstacles([1, 3])
        self.reset_values()

    def reset_values(self):
        """Clears iteration data but PERSISTS current obstacle positions."""
    ...
    def _generate_row_specific_obstacles(self, row_indices):
        """Generates random columns for specific rows, avoiding terminal states."""
        obs = []
        for r in row_indices:
            valid_cols = [c for c in range(self.cols) if (r, c) not in self.terminals]
            if valid_cols:
                obs.append((r, random.choice(valid_cols)))
        return obs

    def get_reward(self, r, c):
        """Formal reward function definition."""
        if (r, c) == (0, 3): return 10.0   # Goal
        if (r, c) == (1, 3): return -10.0  # Trap
        return -0.1                        # Living reward (step cost)

    def get_next_state(self, r, c, action):
        """Standard transition logic with boundary and obstacle checks."""
    ...
    def value_iteration_step(self):
        """Implementation of the Bellman Optimality Equation update."""
    ...
    def policy_iteration_step(self):
        """Two-stage Policy Iteration (Evaluation + Improvement)."""
        # Evaluation
        theta = 0.001
        while True:
            delta_eval = 0
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                best_action = 'UP'; max_q = -float('inf')
                for action in self.actions:
                    ns = self.get_next_state(r, c, action)
        ...
        for r in range(self.rows):
            ...
            for c in range(self.cols):
                if (r, c) in self.terminals: row.append('TERM')
                    ...
                    for action in self.actions:
                        ns = self.get_next_state(r, c, action)
                        if self.V[ns] > max_q: max_q = self.V[ns]; best_action = action
